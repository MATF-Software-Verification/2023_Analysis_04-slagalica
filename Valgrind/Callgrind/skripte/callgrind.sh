...
if [[ $1 = 'cache' ]]; then
	...
	valgrind --tool=callgrind --cache-sim=yes --log-file=$PATH_SERVER_LOG --callgrind-out-file=$PATH_SERVER_REPORT ./server/server &
	valgrind --tool=callgrind --cache-sim=yes --log-file=$PATH_CLIENT1_LOG --callgrind-out-file=$PATH_CLIENT1_REPORT ./slagalica/slagalica &
	valgrind --tool=callgrind --cache-sim=yes --log-file=$PATH_CLIENT2_LOG --callgrind-out-file=$PATH_CLIENT2_REPORT ./slagalica/slagalica &
